...
export default function Login() {
  // Apple SSO disabled - kept in code for future re-enabling if needed
  const { loginWithEmail, registerWithEmail, loginWithGoogle, /* loginWithApple, */ sendPhoneOTP, user } = useAuth();
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem(
          'admin_session',
          JSON.stringify({ role: 'admin', token: data.token, loginTime: new Date().toISOString() })
        );
          ...
          <div className="max-w-md mx-auto">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <button
                onClick={handleGoogle}
                disabled={busy}
                className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-white/[0.05] border border-white/10 text-white hover:bg-white/10 transition-all disabled:opacity-50"
              ...
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M17.05 13.5c-.29 0-.58.22-.58.51 0 .3.29.52.58.52.32 0 .58-.22.58-.52 0-.29-.26-.51-.58-.51zm-4.37 0c-.29 0-.58.22-.58.51 0 .3.29.52.58.52.32 0 .58-.22.58-.52 0-.29-.26-.51-.58-.51zm8.15-3.86c.55 0 1.01-.45 1.01-1 0-.56-.46-1.01-1.01-1.01-.55 0-1 .45-1 1.01 0 .55.45 1 1 1zm-9 0c.54 0 1-.45 1-1 0-.56-.46-1.01-1-1.01-.55 0-1.01.45-1.01 1.01 0 .55.46 1 1.01 1zm8.5-4.64c.64-1.23 1.28-2.45 1.28-3.92 0-4.82-4.13-8.84-9.5-8.84s-9.5 4.02-9.5 8.84c0 1.47.64 2.69 1.28 3.92-.04.53-.09 1.05-.09 1.58 0 3.9 3.4 7.05 7.59 7.05.47 0 .94-.03 1.39-.1.78.19 1.6.29 2.44.29 2.27 0 4.3-.91 5.59-2.38.44-1.08.95-2.27 1.15-3.61-.03-.2-.06-.4-.08-.61.04-.2.07-.4.07-.62 0-.49-.06-1.01-.13-1.54z" />
                </svg>
              ...
              {tab === 'admin' && (
                ...
                >
                  <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 flex gap-3 mb-4">
                    ...
                    <p className="text-amber-200 text-sm">Admin panel access restricted to authorized users only</p>
                  </div>
                  <div>
